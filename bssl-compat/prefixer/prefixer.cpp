...
void MyFrontendAction::EndSourceFileAction() {
  // Write a typedef and extern variable for each function pointer
  {
    ...
    // Generate the header file (ossl.h)
    ...
    hstr << "#ifdef __cplusplus" << std::endl
         ...
         << "}" << std::endl
         << "#endif" << std::endl
         << "void* lookup(const char *symbol);" << std::endl
         << "void ssl_init(void);" << std::endl
         << "void ssl_fini(void);" << std::endl
         << std::endl;
...

  {
    ...
    //Genearate the source code (ossl.c)
    cstr << "//" << std::endl << "// THIS FILE IS GENERATED BY THE PREFIXER TOOL DO NOT EDIT" << std::endl << "//" << std::endl
         ...
         << "#define B_LIBCRYPTO_SO \"b_libcrypto.so" << (m_shlibversion.size() ? "." + m_shlibversion : "") << "\"" << std::endl
         << "#define B_LIBSSL_SO \"b_libssl.so" << (m_shlibversion.size() ? "." + m_shlibversion : "") << "\"" << std::endl
         << std::endl
         << "#define DEFAULT_SSL_IMPL \"ossl" << "\"" << std::endl
         << "#define OSSL_IMPL \"ossl" << "\"" << std::endl
         << "#define BSSL_IMPL \"bssl" << "\"" << std::endl
         << std::endl
         << "static void *libcrypto;" << std::endl
         << "static void *libssl;" << std::endl
         << "static void *boringssl;" << std::endl
         << "struct link_map* map;" << std::endl
         << std::endl
         << "struct " << opt::prefix << "_functions " << opt::prefix << ";" << std::endl
         << std::endl
         << "void ssl_init(void)  __attribute__ ((constructor));" << std::endl
         << "void ssl_fini(void)  __attribute__ ((destructor));" << std::endl
         << std::endl
         << "const char* get_ssl_implementation() {" << std::endl
         << "    const char* impl = getenv(\"SSL_IMPLEMENTATION\");" << std::endl
         ...
         << "  if (fp != NULL) {" << std::endl
         << "    fprintf(stderr, \"Current memory mappings:\\n\");" << std::endl
         << "    while (fgets(buf, sizeof(buf), fp)) {" << std::endl
         << "      fprintf(stderr, \"%s\", buf);" << std::endl
         << "    }" << std::endl
         << "    fclose(fp);" << std::endl
         << "  }" << std::endl
         << "}" << std::endl
         << std::endl
         << "void *lookup(const char *symbol) {" << std::endl                                                                    //lookup function
         << "  void *result;" << std::endl
         << "  const char *s = symbol + 5;" << std::endl
         << "  //fprintf(stderr, \"Looking up symbol: %s\\n\", s);" << std::endl
         << "  if((result = dlsym(libcrypto, s)) != NULL) {" << std::endl
         << "    //fprintf(stderr, \"Found in libcrypto: %s\\n\", s);" << std::endl
         << "    return result;" << std::endl
         << "  }" << std::endl
         << "  if((result = dlsym(libssl, s)) != NULL) {" << std::endl
         << "    //fprintf(stderr, \"Found in libssl: %s\\n\", s);" << std::endl
         << "    return result;" << std::endl
         << "  }" << std::endl
         ...
         << std::endl
         << "  // Check if symbol starts with 'ossl_'" << std::endl
         << "  if (strncmp(symbol, \"ossl_\", 5) == 0) {" << std::endl
         << "    s = symbol + 5;  // Skip 'ossl_' prefix" << std::endl
         << "    has_prefix = 1;" << std::endl
         << "    fprintf(stderr, \"[DEBUG] Looking up %s (original: %s)\\n\", s, symbol);" << std::endl
         << "  }" << std::endl
         << std::endl
         << "  // Try with original symbol name (without prefix)" << std::endl
         << "  result = dlsym(libcrypto, s);" << std::endl
         << "  if (result != NULL) {" << std::endl
         << "    fprintf(stderr, \"[DEBUG] Found %s in libcrypto at %p\\n\", s, result);" << std::endl
         << "    return result;" << std::endl
         << "  }" << std::endl
         << std::endl
         << "  result = dlsym(libssl, s);" << std::endl
         << "  if (result != NULL) {" << std::endl
         << "    fprintf(stderr, \"[DEBUG] Found %s in libssl at %p\\n\", s, result);" << std::endl
         << "    return result;" << std::endl
         << "  }" << std::endl
         << std::endl
         << "  if (has_prefix) {" << std::endl
         << "    // If not found and had prefix, try with full symbol name" << std::endl
         << "    result = dlsym(libcrypto, symbol);" << std::endl
         << "    if (result != NULL) {" << std::endl
         << "      fprintf(stderr, \"[DEBUG] Found %s in libcrypto with prefix at %p\\n\", symbol, result);" << std::endl
         << "      return result;" << std::endl
         << "    }" << std::endl
         << std::endl
         << "    result = dlsym(libssl, symbol);" << std::endl
         << "    if (result != NULL) {" << std::endl
         << "      fprintf(stderr, \"[DEBUG] Found %s in libssl with prefix at %p\\n\", symbol, result);" << std::endl
         << "      return result;" << std::endl
         << "    }" << std::endl
         << "  }" << std::endl
         << std::endl
         ...
         << "}" << std::endl
         << std::endl
         << "void signal_handler(int signo, siginfo_t *info, void *context) {" << std::endl                                         //signal_handler function
         << "  void *array[50];" << std::endl
         << "  int size;" << std::endl
         << "  char **strings;" << std::endl
         << std::endl
         << "  // print signal info" << std::endl
         << "  fprintf(stderr, \"Signal %d (%s) occurred\\n\", signo, strsignal(signo));" << std::endl
         << "  fprintf(stderr, \"Signal code: %d\\n\", info->si_code);" << std::endl
         << "  fprintf(stderr, \"Fault address: %p\\n\", info->si_addr);" << std::endl
         << std::endl
         << "  // print call address" << std::endl
         << "  ucontext_t* uc = (ucontext_t*)context;" << std::endl
         << "  fprintf(stderr, \"Signal occurred at address: %p\\n\", (void*)uc->uc_mcontext.gregs[REG_RIP]);" << std::endl
         << std::endl
         << "  // get backtrace" << std::endl
         << "  size = backtrace(array, 50);" << std::endl
         << "  strings = backtrace_symbols(array, size);" << std::endl
         << std::endl
         << "  fprintf(stderr, \"\\nStack trace (most recent call first):\\n\");" << std::endl
         << "  for (int i = 0; i < size; i++) {" << std::endl
         << "    Dl_info info;" << std::endl
         << "    if (dladdr(array[i], &info)) {" << std::endl
         << "      fprintf(stderr, \"(%d) %s\\n\", i, strings[i]);" << std::endl
         << "      fprintf(stderr, \"    Symbol: %s\\n\", info.dli_sname ? info.dli_sname : \"unknown\");" << std::endl
         << "      fprintf(stderr, \"    Object: %s\\n\", info.dli_fname);" << std::endl
         << "    }" << std::endl
         << "  }" << std::endl
         << std::endl
         << "  free(strings);" << std::endl
         << "  exit(1);" << std::endl
         << "}" << std::endl
         << std::endl
        ...
        //  << "  const char* ssl_impl = get_ssl_implementation();" << std::endl
         ...
         << "  sa.sa_sigaction = signal_handler;" << std::endl
         << "  sa.sa_flags = SA_SIGINFO;" << std::endl
        ...
        //  << "  } else if (!ssl_impl || strncmp(ssl_impl, OSSL_IMPL, 4) == 0) {"                                                        //openssl routine
         ...
         << "    }" << std::endl
         << std::endl
         << "    if((libssl = dlopen(\"/usr/local/openssl-3.2.0/lib64/libssl.so\", RTLD_NOW | RTLD_LOCAL)) == NULL) {" << std::endl
    ...
    cstr << std::endl
        ...
        //  << "  }" << std::endl
         ...
         << "}" << std::endl
         << std::endl;
    cstr << "void ssl_fini(void) {" << std::endl                                                                                       //ssl_fini function
         << "  fprintf(stderr, \"[DEBUG] Destructor called\\n\");" << std::endl
         << "  void* caller = __builtin_return_address(0);" << std::endl
         << "  fprintf(stderr, \"[DEBUG] Called from: %p\\n\", caller);" << std::endl
